...
async function fetchPost() {
  try {
    ...
    const response = await AdminAndManager.getBusinessDetails(businessId);
    businessDetail.value = response.business;
    businessPosts.value = response.posts;
...
const addRejectReason = function () {
  if (rejectReasonsInput.value && rejectFieldInput.value) {
    rejectReasons.value.push({
      description: rejectReasonsInput.value,
      field: toCamelCase(rejectFieldInput.value),
    });
...
const rejectPostBtnClicked = () => {
  if (rejectReasons.value.length !== 0) {
    sendPostEditRequest("reject", businessId);
  } else {
    antMessage.error("Add at leas one reason for rejecting business!");
  }
};
...
const sendPostEditRequest = async function (type, postId) {
  try {
    if (type == "accept") {
      await AdminAndManager.editBusiness(
        { adminStatus: "approved", rejectedReason: [] },
        postId
      );
      antMessage.success("Business approved");
      businessDetail.value.adminStatus = "approved";
    } else if (type == "reject") {
      await AdminAndManager.editBusiness(
        { adminStatus: "rejected", rejectedReason: rejectReasons.value },
        postId
      );
      businessDetail.value.adminStatus = "rejected";
      antMessage.success("Business Rejected");
    }
  } catch (error) {
...
function convertTo12HourFormat(hours24h) {
  // Convert hours to a number
  let hoursNum = parseInt(hours24h);
...
<template>
      ...
      <div class="left-grid">
        <div class="slide-show-and-pagination">
          <div style="position: relative">
            <Swiper
              ...
              :navigation="
                businessDetail?.PropertyPhotos?.length > 1 ? true : false
              "
            >
              <SwiperSlide class="slide-image product">
                <img
                  @click="toggleImageModalShow(businessDetail.logo)"
        ...
        <div class="core-details">
          <div style="position: relative">
            ...
            <span class="title">
              Phone Number : {{ businessDetail?.phoneNumber }}
            </span>
            <span class="title">
              Businss Address : {{ businessDetail?.businessAddress }}
            </span>

            <span class="title">
              Businss Description : {{ businessDetail?.shortDescription }}
            </span>

            <span class="title">
              Opens on :
              <span v-for="day in JSON.parse(businessDetail?.workingTime)">
                {{ day + "   " }}
              </span>
            </span>
            ...
            <span class="title">
              ...
              {{
                convertTo12HourFormat(
                  businessDetail?.workingHours.split(",")[0]
                )
              }}
              -
              {{
                convertTo12HourFormat(
                  businessDetail?.workingHours.split(",")[1]
                )
              }}
            </span>
          </div>
          ...
          <div class="grid-details">
            <template v-for="(unique, key) in businessDetail?.Others">
              <div
                v-if="
                  (Array.isArray(businessDetail?.Others[key]) ||
                    typeof businessDetail?.Others[key] != 'object') &&
                  typeof businessDetail?.Others[key].length !== 0
                "
              ...
              >
                ...
                <!-- values -->
                <span
                  v-if="typeof businessDetail?.Others[key] == 'object'"
                  class="value"
                >
                  <template
                    v-for="(item, index, parent) in businessDetail?.Others[key]"
                  >
                ...
                <span
                  v-if="typeof businessDetail?.Others[key] == 'string'"
                  class="value"
                >
                  {{ businessDetail?.Others[key] }}
                </span>
                <span
                  v-if="typeof businessDetail?.Others[key] == 'boolean'"
                  class="value"
                >
                  {{ businessDetail?.Others[key] == true ? "Yes" : "No" }}
                </span>
                <span v-if="key == 'SecuredParking'" class="value"> Yes </span>

                <span
                  v-if="typeof businessDetail?.Others[key] == 'number'"
                  class="value"
                >
                  <span v-if="key == 'PropertySize'">
                    {{ businessDetail?.Others[key] }} sqm
                  </span>
                  <span v-else-if="key == 'ServiceCharge'">
                    ₦ {{ formartPrice(businessDetail?.Others[key]) }}
                  </span>
                  <span v-else>
                    {{ businessDetail?.Others[key] }}
                  </span>
          ...
          <span class="description"> {{ businessDetail?.Description }} </span>
        </div>
        ...
        >
          <a-button
            ...
            type="primary"
            @click="sendPostEditRequest('accept', businessId)"
            >Approve Business</a-button
          >
          <a-button
            style="background-color: red; border-color: red"
            type="primary"
            @click="rejectModalVisible = true"
            >Reject Business</a-button
          >
        </div>
        <div style="width: 100%; padding: 0 15px; margin-top: 20px">
          <h4>Business Posts</h4>
          <div class="">
            <template v-for="(card, index) in businessPosts">
      ...
      <form @submit.prevent="addRejectReason">
        <div style="display: flex" class="">
          ...
          <datalist id="reject-reason-keys">
            ...
            <option value="Business Address"></option>
            <option value="Business Name"></option>
            <option value="Phone Number"></option>
            <option value="Short Description"></option>
  ...
  >
    <span
      v-if="businessDetail?.adminStatus === 'approved'"
      class="badge badge-pill badge-soft-success"
      style="font-size: large"
      >This Business is Active</span
    >
    <span
      v-if="businessDetail?.adminStatus === 'pending'"
      class="badge badge-pill badge-soft-warning"
      style="font-size: large"
      >This Business is Pending</span
    >
    <span
      v-if="businessDetail?.adminStatus === 'rejected'"
      class="badge badge-pill badge-soft-danger"
      style="font-size: large"
      >This Business is Rejected</span
    >
  </div>
</template>
...
.other-details .left-grid .big-price {
  display: none;
}
.other-details .left-grid .owner a {
  display: flex;
  align-items: center;
  margin-left: 10px;
}
.other-details .left-grid .owner a .dp {
  position: relative;
  margin-right: 15px;
}
.other-details .left-grid .owner a .dp svg,
.other-details .left-grid .owner a .dp img {
  height: 40px;
  width: 40px;
  border-radius: 50%;
  color: #777;
...
.other-details .left-grid .owner a .dp .active {
  height: 7px;
  width: 7px;
  position: absolute;
  border-radius: 50%;
...
.other-details .left-grid .owner a .details {
  display: flex;
  flex-direction: column;
  justify-content: center;
  width: 100%;
}
.other-details .left-grid .owner a .details .name-ad-count {
  display: flex;
  align-items: center;
  position: relative;
  margin-bottom: 5px;
...
.other-details .left-grid .owner a .details .name-ad-count span {
  font-size: 18px;
  font-weight: bold;
  margin-right: 15px;
}
.other-details .left-grid .owner a .details .name-ad-count span.count {
  color: #4ea54c;
}
.other-details .left-grid .owner a .details .name-ad-count svg {
  height: 30px;
  position: absolute;
  right: 10px;
}
.other-details .left-grid .owner-contact button {
  height: 40px;
  width: 95%;
...
.other-details .left-grid .owner-contact button a {
  font-size: 15px;
  color: #fff;
}
.other-details .left-grid .big-price span {
  font-size: 30px;
  margin-bottom: 5px;
  font-weight: bold;
}
...
@media only screen and (min-width: 768px) {
  ...
  .other-details .left-grid .owner-contact {
    background: #fff;
    display: flex;
    flex-direction: column;
    align-items: center;
  }
  .other-details .left-grid .owner {
    padding: 15px;
    width: 100%;
  }
  .other-details .left-grid .owner a .dp {
